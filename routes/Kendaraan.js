...
module.exports = (router) => {
  router.post("/kendaraan/create", fields, async (req, res) => {
    const kendaraan = JSON.parse(req.body.kendaraan);
    const pengujian = JSON.parse(req.body.pengujian);
  ...
  router.get("/kendaraan/get", async (req, res) => {
    kendaraanController.getdata().then((result) => {
      res.json(result);
    });
  });
